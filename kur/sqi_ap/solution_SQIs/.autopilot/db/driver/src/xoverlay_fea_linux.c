...
typedef struct {
    ...
    XOverlay_fea_uio_map maps[ MAX_UIO_MAPS ];
} XOverlay_fea_uio_info;

/***************** Variable Definitions **************************************/
static XOverlay_fea_uio_info uio_info;
...
static int uio_info_read_name(XOverlay_fea_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XOverlay_fea_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XOverlay_fea_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XOverlay_fea_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XOverlay_fea_Initialize(XOverlay_fea *InstancePtr, const char* InstanceName) {
	XOverlay_fea_uio_info *InfoPtr = &uio_info;
