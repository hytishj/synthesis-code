...
`define AUTOTB_DUT      overlay_fea
`define AUTOTB_DUT_INST AESL_inst_overlay_fea
`define AUTOTB_TOP      apatb_overlay_fea_top
`define AUTOTB_LAT_RESULT_FILE "overlay_fea.result.lat.rb"
`define AUTOTB_PER_RESULT_TRANS_FILE "overlay_fea.performance.result.transaction.xml"
`define AUTOTB_TOP_INST AESL_inst_apatb_overlay_fea_top
...
`define AESL_DEPTH_kSQI 1
`define AESL_DEPTH_ap_local_deadlock 1
`define AUTOTB_TVIN_gmem  "./c.overlay_fea.autotvin_gmem.dat"
`define AUTOTB_TVIN_data  "./c.overlay_fea.autotvin_data.dat"
`define AUTOTB_TVIN_length_r  "./c.overlay_fea.autotvin_length_r.dat"
`define AUTOTB_TVIN_gmem_out_wrapc  "./rtl.overlay_fea.autotvin_gmem.dat"
`define AUTOTB_TVIN_data_out_wrapc  "./rtl.overlay_fea.autotvin_data.dat"
`define AUTOTB_TVIN_length_r_out_wrapc  "./rtl.overlay_fea.autotvin_length_r.dat"
`define AUTOTB_TVOUT_kSQI  "./c.overlay_fea.autotvout_kSQI.dat"
`define AUTOTB_TVOUT_kSQI_out_wrapc  "./impl_rtl.overlay_fea.autotvout_kSQI.dat"
module `AUTOTB_TOP;

parameter AUTOTB_TRANSACTION_NUM = 1;
parameter PROGRESS_TIMEOUT = 10000000;
parameter LATENCY_ESTIMATION = 22089;
parameter LENGTH_gmem = 2000;
parameter LENGTH_data = 1;
parameter LENGTH_length_r = 1;
parameter LENGTH_kSQI = 1;
...
reg AESL_REG_kSQI_ap_vld = 0;
// The signal of port kSQI
reg [31: 0] AESL_REG_kSQI = 0;
always @(posedge AESL_clock)
begin
    if(AESL_reset === 0)
        AESL_REG_kSQI = 0; 
    else if(kSQI_ap_vld) begin
        AESL_REG_kSQI <= kSQI;
        AESL_REG_kSQI_ap_vld <= 1;
    end
end 

initial begin : write_file_process_kSQI
    integer fp;
    integer fp_size;
    integer err;
    integer ret;
    ...
    reg [8 * 5:1] str;
    wait(AESL_reset === 1);
    fp = $fopen(`AUTOTB_TVOUT_kSQI_out_wrapc,"w");
    if(fp == 0) begin       // Failed to open file
        $display("Failed to open file \"%s\"!", `AUTOTB_TVOUT_kSQI_out_wrapc);
        $display("ERROR: Simulation using HLS TB failed.");
        $finish;
    end
    $fdisplay(fp,"[[[runtime]]]");
    ...
    while (transaction_idx != AUTOTB_TRANSACTION_NUM) begin
        ...
        if(AESL_REG_kSQI_ap_vld)  begin
          $fdisplay(fp,"0x%x", AESL_REG_kSQI);
        AESL_REG_kSQI_ap_vld = 0;
        end
...
initial begin : generate_AESL_ready_cnt_proc
    ...
    fp1 = $fopen("./rtl.overlay_fea.autotvout_kSQI.dat", "r");
    fp2 = $fopen("./impl_rtl.overlay_fea.autotvout_kSQI.dat", "r");
    if(fp1 == 0)        // Failed to open file
        $display("Failed to open file \"./rtl.overlay_fea.autotvout_kSQI.dat\"!");
    else if(fp2 == 0)
        $display("Failed to open file \"./impl_rtl.overlay_fea.autotvout_kSQI.dat\"!");
    else begin
        $display("Comparing rtl.overlay_fea.autotvout_kSQI.dat with impl_rtl.overlay_fea.autotvout_kSQI.dat");
        post_check(fp1, fp2);
    end
    $fclose(fp1);
    $fclose(fp2);
