...
float bassqi_calc(float* power_spec)
{
	float vlf_bas, total_bas=0;
	vlf_bas = power_spec[0] + power_spec[1];

	Loop_bas:for(int k=0;k<41;k++)
	{
		total_bas += power_spec[k];
	}

	return vlf_bas / total_bas;
}

void overlay_fea(float* data, int length, float* basSQI)
{
	int N=2048, M=11;
